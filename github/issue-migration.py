...
def post_req(url, data, credentials):
    r = requests.post(url=url, data=data, auth=(credentials['user_name'], credentials['token']),
                      headers={'Content-type': 'application/json', 'Accept': 'application/vnd.github.v3.html+json'})
    return r
...
def create_milestones(milestones, destination_url, destination, credentials):
    ...
    for milestone in milestones:
        ...
        if milestone["due_on"]:
            milestone_prime["due_on"] = milestone["due_on"]
    ...
    for issue in issues:
        ...
        if not status:
            ...
            if message['errors'][0]['code'] == 'invalid' and message['errors'][0]['field'] == 'assignee':
                sys.stderr.write("WARNING: Assignee " + message['errors'][0]['value'] + " on issue \"" + issue_prime[
                    'title'] + "\" does not exist in the destination repository. Issue added without assignee field.\n\n")
...
def main():
    ...
    if args.issues:
        ...
        if issues:
            ...
            if (args.sourceRoot == args.destinationRoot):
                sameInstall = True
            create_issues(issues, destination_root, destination_repo, args.milestones, args.labels, milestone_map,
